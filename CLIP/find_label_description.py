...
def count_rectangle_labels(
...
) -> Tuple[Counter, int, int, int, int, int, List[str]]:
    """
    ...
    Returns:
        ...
        - Count of files with label pair co-occurrence
        - Number of processed files 
        - Number of skipped files
        - Number of files with parsing errors
        - Number of tasks with no rectangle labels
        - List of filenames containing "Popis v textu" label
    """
    ...
    if not os.path.isdir(jsons_dir_path):
        ...
        return None, 0, processed_files, skipped_files, errors_parsing, tasks_with_no_rect_labels, files_with_popis_v_textu
    ...
    if not filenames:
        ...
        return label_counts, pair_count, processed_files, skipped_files, errors_parsing, tasks_with_no_rect_labels, files_with_popis_v_textu
    ...
    if show_progress:
        try:
            ...
            try:
                ...
                file_iterator = tqdm(filenames, desc="Analyzing JSON files", unit="file")
        ...
        try:
                        ...
                        if isinstance(labels_list, list) and labels_list:
                            ...
                            for label_name in labels_list:
                                if isinstance(label_name, str) and label_name.strip():
                                    ...
                                    # Check if this file has "Popis v textu" label
                                    if clean_label_name == "Popis v textu":
                                        has_popis_v_textu = True
                                else:
                                    logging.warning(f"Invalid label found in {filename}: {label_name}")

            # Add filename to list if it has "Popis v textu" label
            if has_popis_v_textu:
                files_with_popis_v_textu.append(filename)
            ...
            if show_progress and hasattr(file_iterator, 'set_postfix'):
                file_iterator.set_postfix({
                    ...
                    'descriptions': len(files_with_popis_v_textu),
...
def run_find_descriptions(
    jsons_dir: str = DEFAULT_JSONS_DIR_NAME, 
    pair_to_check: Optional[List[str]] = None,
    output_file: str = "popis_v_textu_files.txt",
    print_table: bool = True,
    show_progress: bool = True
) -> Dict[str, Any]:
    """
    Main function to analyze JSON files for rectangle labels and descriptions.
    ...
    Args:
        ...
        output_file: Filename to write the list of files with "Popis v textu"
    ...
    logging.info(f"Starting analysis for directory: {jsons_dir_abs}")

    # Run the label counting function
    label_counts, pair_count_result, processed_count, skipped_count, errors_count, no_labels_count, files_with_popis = count_rectangle_labels(
        jsons_dir_abs,
        pair_to_check,
        show_progress
    )
    ...
    results = {
        ...
        "files_with_description": files_with_popis,
        "description_count": len(files_with_popis)
    }
    ...
    # Write files with "Popis v textu" label to output file
    output_file_path = os.path.join(script_dir, output_file)
    if files_with_popis:
        try:
            with open(output_file_path, 'w', encoding='utf-8') as f:
                for filename in files_with_popis:
                    f.write(f"{filename}\n")
            logging.info(f"List of files with 'Popis v textu' label written to: {output_file_path}")
            results["description_file"] = output_file_path
        except IOError as e:
            error_msg = f"Error writing file list to {output_file_path}: {e}"
            logging.error(error_msg)
            results["description_file_error"] = error_msg
    else:
        logging.info("No files with 'Popis v textu' label were found.")
    ...
    if print_table and label_counts is not None:
        ...
        logging.info(f"Files with 'Popis v textu' label: {results['description_count']}")
        logging.info("--------------------------")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"Find and analyze label descriptions in annotation JSON files (v{SCRIPT_VERSION}).",
    ...
    parser.add_argument(
        "--output",
        default="popis_v_textu_files.txt",
        help="Output file name for the list of files with descriptions"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    result = run_find_descriptions(
        jsons_dir=args.jsons_dir,
        pair_to_check=args.pair,
        output_file=args.output,
        print_table=True
    )
    
    logging.info("Label description analysis script finished.")