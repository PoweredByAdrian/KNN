...
def count_rectangle_labels(jsons_dir_path, label_pair_to_check=None):
    ...
    Returns a tuple: (label_counts, pair_count, processed_files, skipped_files, errors_parsing, tasks_with_no_rect_labels, files_with_popis_v_textu)
    """
    ...
    # Add a list to collect files with "Popis v textu"
...
if __name__ == "__main__":
    ...
    if label_counts is not None:
        ...
        # Write files with "Popis v textu" label to a text file
        output_file = os.path.join(script_dir, "popis_v_textu_files.txt")
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                for filename in files_with_popis:
                    f.write(f"{filename}\n")
            logging.info(f"List of files with 'Popis v textu' label written to: {output_file}")
        except IOError as e:
            logging.error(f"Error writing file list to {output_file}: {e}")
