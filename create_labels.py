...
class_names = ["popis u obrázku","Obrázek"]  # only one class
class_ids = [0,1]
# CLASS_ID = 0  # Index for "popis u obrázku"

# Function to convert bbox from percentage to YOLO format
def convert_to_yolo_format(x_percent, y_percent, width_percent, height_percent, img_width, img_height,class_id):
        ...
        for annotation in annotations:
            for result in annotation["result"]:
                if result.get("type") == "rectanglelabels" and "Popis u obrázku" in result.get("value", {}).get("rectanglelabels", []):
                    ...
                    yolo_format = convert_to_yolo_format(data['x'],data['y'],data['width'],data['height'],width,height,class_ids[0])
                    yolo_file.write(yolo_format+"\n")
                elif result.get("type") == "rectanglelabels" and "Obrázek" in result.get("value", {}).get("rectanglelabels", []):
                    data = result["value"]
                    ...
                    yolo_format = convert_to_yolo_format(data['x'],data['y'],data['width'],data['height'],width,height,class_ids[1])
                    yolo_file.write(yolo_format+"\n")
                else:
                    pass
